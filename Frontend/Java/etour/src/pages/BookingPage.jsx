...
function BookingPage() {
  ...
  const [errors, setErrors] = useState([{}]); // Parallel array for passenger form errors
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8081/api/costs/package/${packageId}`)
      .then(res => res.json())
      .then(data => {
        setCostData({
  ...
  const validatePassenger = (pax) => {
    ...
    if (!pax.firstName.trim() || pax.firstName.length < 2 || !/^[A-Za-z]+$/.test(pax.firstName)) {
      newErrors.firstName = "First name must be at least 2 letters and only alphabets.";
    }

    if (!pax.lastName.trim() || pax.lastName.length < 2 || !/^[A-Za-z]+$/.test(pax.lastName)) {
      newErrors.lastName = "Last name must be at least 2 letters and only alphabets.";
    }
  ...
  const handleChange = (index, e) => {
    ...
    // Live validation for the changed passenger
