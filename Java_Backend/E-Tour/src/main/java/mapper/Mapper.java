...
public class Mapper {
    ...
    public static PackageDTO MapToPackageDTO(TourPackage pkg) {
	    ...
	    // Category mapping for special categories
	    if (pkg.getSubcategory() == null && pkg.getCategory() != null) {
	        dto.setCategoryId(pkg.getCategory().getCategoryId());
	        dto.setCategoryName(pkg.getCategory().getCategoryName());
	    }
    ...
    public static TourPackage MapToPackage(PackageDTO dto, Subcategory subcategory, Category category) {
	    if (dto == null) return null;
	    ...
	    // Assign either subcategory or category
	    ...
	    if (dto.getCosts() != null) {
	        pkg.setCosts(dto.getCosts().stream()
	            .map(costDTO -> {
	                Cost cost = Mapper.mapToCost(costDTO);
	                cost.setTourPackage(pkg);
	                return cost;
	            }).collect(Collectors.toList()));
	    }
